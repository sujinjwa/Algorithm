...
for i in range(19):
    for j in range(19):
        ...
        for k in range(8): # 8가지의 방향의 dx, dy의 인덱스 := k
            for l in range(1, 5): # (i, j)에서 앞으로 나아가는 횟수 := l
                x, y = i + dx[k] * l, j + dy[k] * l # 8가지의 방향 중 4번씩 앞으로 나아갔을 때 4가지의 각각 위치
                
                if in_range(x, y) == False:
                    break
                if is_same_color(i, j, x, y) == False:
                    break
                    
                if l == 4: # 5개의 바둑알 모두 오목판 벗어나지 않고, 같은 색상인 경우
                    print(arr[i][j]) # 해당 위치의 색상 출력
                    print(i + (2 * dx[k]) + 1, j + (2 * dy[k]) + 1) # 행과 열 숫자 구해야 하므로 index에서 1씩 더해줘야 함
                    sys.exit()
